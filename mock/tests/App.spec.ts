...
// Checking mode switch between brief and verbose
test("after I click the button, mode changes", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Mode: Brief" })
  ).toBeVisible();
...
/**
 * Test if Command Input is Cleared after Submit
 */
test("after I click the button, the command input is cleared", async ({
  page,
}) => {
  await page.goto("http://localhost:8000/");
